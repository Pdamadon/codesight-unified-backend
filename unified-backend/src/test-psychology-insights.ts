...
import { PsychologyInsightsService } from './services/psychology-insights';
import { OpenAIIntegrationService } from './services/openai-integration';

async function testPsychologyInsights() {
  const prisma = new PrismaClient();
  const openaiService = new OpenAIIntegrationService(prisma);
  const psychologyService = new PsychologyInsightsService(prisma, openaiService);
