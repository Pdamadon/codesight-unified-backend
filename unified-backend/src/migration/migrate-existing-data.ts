...
export class DataMigrationService {
  ...
  constructor(config: MigrationConfig) {
    this.sourceDb = new Pool({ connectionString: config.sourceDatabase });
    this.targetDb = new PrismaClient({ datasourceUrl: config.targetDatabase });
    this.logger = new Logger('DataMigration');
  }

  async migrateAllData(config: MigrationConfig): Promise<MigrationResult> {
  ...
  private async migrateSessions(config: MigrationConfig): Promise<{count: number, errors: string[], warnings: string[]}> {
  ...
  private async migrateInteractions(config: MigrationConfig): Promise<{count: number, errors: string[]}> {
  ...
  private async convertToUnifiedInteraction(sessionId: string, event: any): Promise<any> {
  ...
  private calculateQualityScore(session: any): number {
    let score = 0;
    
    // Base score for having session data
    if (session.session_data) score += 20;
    
    // Audio recording bonus
    if (session.audio_url) score += 25;
    
    // Video recording bonus (legacy)
    if (session.video_url) score += 20;
    
    // Transcription bonus
    if (session.transcription) score += 15;
    
    // Analysis data bonus
    if (session.analysis) score += 10;
    
    // Session duration bonus
    const duration = this.calculateDuration(session);
    if (duration > 300) score += 10; // 5+ minutes
  ...
  private calculateCompleteness(session: any): number {
    ...
    if (sessionData.events && sessionData.events.length > 0) completeness += 40;
    if (session.audio_url) completeness += 30;
    if (session.transcription) completeness += 20;
    if (sessionData.summary) completeness += 10;
  ...
  private calculateReliability(session: any): number {
    ...
    if (events.length > 10) reliability += 20;
    else if (events.length > 5) reliability += 10;
    
    // Consistent data structure
    const hasConsistentData = events.every((e: any) => e.data && e.data.timestamp);
    if (hasConsistentData) reliability += 20;
  ...
  private mapInteractionType(type: string): string {
