...
exports.archiveRoutes = router;
const logger = new logger_1.Logger('ArchiveRoutes');
...
router.get('/', [
    ...
    (0, express_validator_1.query)('status').optional().isIn(['CREATING', 'UPLOADING', 'COMPLETED', 'FAILED']),
...
router.post('/create/:sessionId', [
    ...
    try {
        ...
        if ((0, type_helpers_1.getErrorMessage)(error) === 'Session not found') {
            return res.status(404).json({
                success: false,
                error: 'Session not found'
            });
        }
        if ((0, type_helpers_1.getErrorMessage)(error) === 'Archive already exists for this session') {
...
router.get('/:id/download', [
    (0, express_validator_1.param)('id').isUUID(),
    (0, express_validator_1.query)('expiresIn').optional().isInt({ min: 300, max: 86400 }) // 5 minutes to 24 hours
], handleValidationErrors, async (req, res) => {
    try {
        const { id } = req.params;
...
router.get('/session/:sessionId', [
    ...
    try {
        ...
        const serializedArchives = archives.map(archive => ({
            ...archive,
            fileSize: Number(archive.fileSize),
            manifest: archive.manifest ? JSON.parse(archive.manifest) : null
        }));
        res.json({
            success: true,
            data: {
...
router.get('/stats/overview', async (req, res) => {
    try {
        const [totalArchives, completedArchives, failedArchives, archiveStats, recentArchives] = await Promise.all([
