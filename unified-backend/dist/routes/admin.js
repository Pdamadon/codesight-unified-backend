...
router.get('/dashboard', async (req, res) => {
    try {
        const [sessionStats, interactionStats, screenshotStats, trainingStats, qualityStats] = await Promise.all([
            // Session statistics
...
router.get('/quality/overview', async (req, res) => {
    try {
        const qualityService = new quality_control_clean_1.QualityControlService();
    ...
    try {
        ...
        res.json({
            ...
            data: {
                ...
                averageQuality: reports.reduce((sum, r) => sum + r.score, 0) / reports.length,
                qualityDistribution: {
                    trainingReady: reports.filter((r) => r.isValid).length,
                    needsImprovement: reports.filter((r) => !r.isValid).length
                }
            }
        });
    }
...
router.get('/storage/overview', async (req, res) => {
    try {
        const [screenshotStats, archiveStats, sessionStats] = await Promise.all([
            prisma.screenshot.aggregate({
                _count: true,
                _sum: { fileSize: true }
            }),
...
router.post('/storage/cleanup', [
    (0, express_validator_1.query)('olderThanDays').optional().isInt({ min: 1, max: 365 })
], handleValidationErrors, async (req, res) => {
    try {
...
router.get('/training/overview', async (req, res) => {
    try {
        const [trainingStats, modelStats, recentJobs] = await Promise.all([
            prisma.trainingData.groupBy({
                by: ['status'],
                _count: true,
