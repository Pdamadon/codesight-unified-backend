...
const monitoring_analytics_1 = require("../services/monitoring-analytics");
const logger_1 = require("../utils/logger");
const router = (0, express_1.Router)();
exports.analyticsRoutes = router;
const logger = new logger_1.Logger('AnalyticsRoutes');
const monitoringService = new monitoring_analytics_1.MonitoringAnalyticsService();
// Dashboard overview endpoint
router.get('/dashboard', async (req, res) => {
    try {
        const timeRange = req.query.timeRange || '24h';
        const [businessMetrics, performanceAnalytics, systemAnalytics] = await Promise.all([
...
router.get('/quality', async (req, res) => {
    try {
        ...
        const analytics = {
            ...
            averageQuality: calculateAverage(qualityReports.map((r) => r.overallScore)),
            averageCompleteness: calculateAverage(qualityReports.map((r) => r.completenessScore)),
            averageReliability: calculateAverage(qualityReports.map((r) => r.reliabilityScore)),
            averageAccuracy: calculateAverage(qualityReports.map((r) => r.accuracyScore)),
...
router.get('/training', async (req, res) => {
    try {
        ...
        const analytics = {
            ...
            completedJobs: trainingData.filter((t) => t.status === 'COMPLETED').length,
            failedJobs: trainingData.filter((t) => t.status === 'FAILED').length,
            averageQuality: calculateAverage(trainingData.map((t) => t.trainingQuality)),
            averagePerformance: calculateAverage(trainingData.map((t) => t.expectedPerformance)),
...
router.get('/export/:type', async (req, res) => {
    try {
        ...
        switch (type) {
            case 'performance':
                data = await monitoringService.getPerformanceAnalytics(timeRange);
                break;
            case 'business':
                data = await monitoringService.getBusinessMetrics();
                break;
            case 'system':
                data = await monitoringService.getSystemAnalytics(timeRange);
                break;
            default:
                return res.status(400).json({
                    success: false,
...
function calculateQualityDistribution(reports) {
...
function calculateQualityTrends(reports) {
    if (reports.length < 2)
...
function calculateStorageCost(totalSize) {
...
function calculateCompressionSavings(stats) {
...
function calculateStorageOptimization(stats) {
    return {
...
async function generateQualityReport(timeRange, filters) {
