...
exports.PsychologyInsightsService = void 0;
const logger_1 = require("../utils/logger");
class PsychologyInsightsService {
    prisma;
    logger;
    openaiService;
    ...
    trustSignals = new Map();
    constructor(prisma, openaiService) {
        this.prisma = prisma;
        this.logger = new logger_1.Logger("PsychologyInsights");
        this.openaiService = openaiService;
        this.initializePsychologyPatterns();
    }
    initializePsychologyPatterns() {
        // Emotional triggers
    ...
    async extractUserPsychologyInsights(sessionId) {
        try {
            ...
            const psychologyProfile = await this.synthesizePsychologyProfile(sessionId, visualPsychology, behavioralPsychology, decisionPatterns, session);
    ...
    async analyzeVisualPsychology(screenshots) {
        ...
        try {
            ...
            const visionAnalysis = await this.openaiService.analyzeScreenshotsAdvanced(screenshots, {
    ...
    async analyzeBehavioralPsychology(interactions) {
    ...
    // Analyze decision-making patterns
    analyzeDecisionMakingPatterns(session) {
        const interactions = session.interactions || [];
        // Identify key decision points
        const decisionPoints = interactions.filter((interaction) => this.isDecisionPoint(interaction));
    ...
    // Synthesize comprehensive psychology profile
    async synthesizePsychologyProfile(sessionId, visualPsychology, behavioralPsychology, decisionPatterns, session) {
        // Determine dominant personality
        const dominantPersonality = this.determineDominantPersonality(behavioralPsychology.personalityTraits, decisionPatterns);
        ...
        const insights = await this.generateDetailedInsights(visualPsychology, behavioralPsychology, decisionPatterns);
        // Generate behavior predictions
        const behaviorPredictions = this.generateBehaviorPredictions(dominantPersonality, emotionalState, decisionMakingStyle, { trustLevel, urgencyLevel, pricesensitivity, socialInfluence });
        // Generate recommendations
        const recommendations = this.generateRecommendations(dominantPersonality, emotionalState, insights, behaviorPredictions);
        // Calculate overall confidence
        const confidence = this.calculateOverallConfidence(visualPsychology, behavioralPsychology, session.interactions?.length || 0);
        return {
            sessionId,
            dominantPersonality,
            emotionalState,
    ...
    extractVisualTriggers(visionAnalysis) {
        const triggers = [];
    ...
    analyzeColorPsychology(visionAnalysis) {
    ...
    analyzeLayoutInfluence(visionAnalysis) {
        const analysisText = JSON.stringify(visionAnalysis).toLowerCase();
        return {
    ...
    identifyTrustElements(visionAnalysis) {
        const trustElements = [];
        const analysisText = JSON.stringify(visionAnalysis).toLowerCase();
    ...
    identifyPersuasionTechniques(visionAnalysis) {
        const techniques = [];
        const analysisText = JSON.stringify(visionAnalysis).toLowerCase();
    ...
    analyzeNavigationStyle(interactions) {
        ...
        if (backtrackCount > explorationDepth * 0.3)
            return 'exploratory';
        if (explorationDepth < 5)
            return 'focused';
        if (backtrackCount < explorationDepth * 0.1)
            return 'linear';
        return 'mixed';
    }
    ...
    calculateInteractionIntensity(interactions) {
    ...
    calculateDecisionSpeed(interactions) {
        ...
        if (averageDecisionTime < 30000)
            return 'fast'; // < 30 seconds
        if (averageDecisionTime < 120000)
            return 'medium'; // < 2 minutes
        return 'slow';
    }
    calculateExplorationDepth(interactions) {
    ...
    identifyPersonalityTraits(behaviorMetrics) {
        const traits = {
            analytical: 0,
            impulsive: 0,
            cautious: 0,
        ...
        // Analytical traits
        if (behaviorMetrics.explorationDepth > 5)
            traits.analytical += 30;
        if (behaviorMetrics.decisionSpeed === 'slow')
            traits.analytical += 20;
        // Impulsive traits
        if (behaviorMetrics.decisionSpeed === 'fast')
            traits.impulsive += 40;
        if (behaviorMetrics.interactionIntensity > 10)
            traits.impulsive += 20;
        // Cautious traits
        if (behaviorMetrics.navigationStyle === 'exploratory')
            traits.cautious += 30;
        if (behaviorMetrics.timeSpentPatterns.averageInterval > 5000)
            traits.cautious += 20;
        // Practical traits
        if (behaviorMetrics.navigationStyle === 'focused')
            traits.practical += 30;
        if (behaviorMetrics.explorationDepth < 3)
            traits.practical += 20;
        return traits;
    }
    inferEmotionalState(behaviorMetrics, interactions) {
        ...
        if (elementTexts.includes('sale') || elementTexts.includes('discount')) {
            if (behaviorMetrics.interactionIntensity > 8)
                return 'excited';
        }
    ...
    determineDecisionMakingStyle(behaviorMetrics) {
    ...
    calculatePriceInfluence(interactions) {
        const priceInteractions = interactions.filter(i => {
            const text = i.elementText?.toLowerCase() || '';
            return this.behavioralIndicators.get('price_conscious')?.some(keyword => text.includes(keyword));
        });
        return Math.min((priceInteractions.length / interactions.length) * 100, 100);
    }
    calculateSocialProofInfluence(interactions) {
    ...
    calculateUrgencyInfluence(interactions) {
    ...
    calculateTrustInfluence(interactions) {
    ...
    calculateConvenienceInfluence(interactions) {
    ...
    calculateTrustLevel(visualPsychology, behavioralPsychology) {
        ...
        if (behavioralPsychology.decisionMakingStyle === 'deliberate')
            trustLevel += 15;
        if (behavioralPsychology.emotionalState === 'confident')
            trustLevel += 20;
        return Math.min(trustLevel, 100);
    }
    calculateUrgencyLevel(visualPsychology, decisionPatterns) {
        let urgencyLevel = 30; // Base urgency
        // Visual urgency triggers
        if (visualPsychology.visualTriggers.includes('urgency'))
            urgencyLevel += 30;
        if (visualPsychology.visualTriggers.includes('fear_of_missing_out'))
            urgencyLevel += 25;
        // Decision speed influence
        urgencyLevel += decisionPatterns.decisionFactors.urgency * 0.4;
        return Math.min(urgencyLevel, 100);
    }
    calculatePriceSensitivity(behavioralPsychology, decisionPatterns) {
        let sensitivity = 40; // Base sensitivity
        // Behavioral indicators
        const traits = behavioralPsychology.personalityTraits;
        if (traits.cautious > traits.impulsive)
            sensitivity += 20;
        // Decision factors
        sensitivity += decisionPatterns.decisionFactors.price * 0.6;
        return Math.min(sensitivity, 100);
    }
    calculateSocialInfluence(visualPsychology, behavioralPsychology) {
        let influence = 35; // Base influence
        // Visual social proof
        if (visualPsychology.visualTriggers.includes('social_proof'))
            influence += 25;
        // Behavioral social indicators
        if (behavioralPsychology.personalityTraits.social > 50)
            influence += 20;
        return Math.min(influence, 100);
    }
    // Generate detailed insights
    async generateDetailedInsights(visualPsychology, behavioralPsychology, decisionPatterns) {
        const insights = [];
        // Emotional insights
        ...
        const dominantTrait = Object.entries(behavioralPsychology.personalityTraits)
            .sort(([, a], [, b]) => b - a)[0];
        if (dominantTrait && dominantTrait[1] > 50) {
            insights.push({
                category: 'cognitive',
                factor: 'personality_trait',
                confidence: dominantTrait[1],
    ...
    // Generate behavior predictions
    generateBehaviorPredictions(personality, emotion, decisionStyle, metrics) {
        const predictions = [];
        ...
        predictions.push(...(personalityPredictions[personality] || []));
    ...
    // Generate recommendations
    generateRecommendations(personality, emotion, insights, predictions) {
        ...
        recommendations.push(...(personalityRecommendations[personality] || []));
    ...
    // Calculate overall confidence
    calculateOverallConfidence(visualPsychology, behavioralPsychology, interactionCount) {
        ...
        if (visualPsychology.visualTriggers.length > 0)
            confidence += 15;
        if (visualPsychology.trustElements.length > 0)
            confidence += 10;
        // Behavioral analysis quality
        const maxTrait = Math.max(...Object.values(behavioralPsychology.personalityTraits).map((v) => v));
        if (maxTrait > 50)
            confidence += 10;
        return Math.min(confidence, 100);
    }
    // Utility methods
    calculateAverageClickTime(clickInteractions) {
        if (clickInteractions.length < 2)
            return 0;
        const intervals = [];
        for (let i = 1; i < clickInteractions.length; i++) {
            intervals.push(Number(clickInteractions[i].timestamp) - Number(clickInteractions[i - 1].timestamp));
        }
        return intervals.reduce((a, b) => a + b, 0) / intervals.length;
    }
    analyzeClickDistribution(clickInteractions) {
    ...
    identifyHesitationPatterns(clickInteractions) {
    ...
    countBacktracking(navigationEvents) {
    ...
    calculateAverageDecisionTime(decisionPoints) {
        if (decisionPoints.length < 2)
            return 0;
        const intervals = [];
        for (let i = 1; i < decisionPoints.length; i++) {
            intervals.push(Number(decisionPoints[i].timestamp) - Number(decisionPoints[i - 1].timestamp));
        }
        return intervals.reduce((a, b) => a + b, 0) / intervals.length;
    }
    calculateOverallDecisionSpeed(interactions) {
        const decisionPoints = interactions.filter(i => this.isDecisionPoint(i));
        if (decisionPoints.length === 0)
            return 0;
    ...
    identifyDecisionBarriers(interactions) {
        const barriers = [];
        const elementTexts = interactions.map(i => i.elementText?.toLowerCase() || '').join(' ');
        // Common decision barriers
        if (elementTexts.includes('out of stock'))
            barriers.push('availability');
        if (elementTexts.includes('shipping') && elementTexts.includes('cost'))
            barriers.push('shipping_cost');
        if (elementTexts.includes('return') && elementTexts.includes('policy'))
            barriers.push('return_policy');
        if (elementTexts.includes('size') && elementTexts.includes('guide'))
            barriers.push('sizing_uncertainty');
        return barriers;
    }
    identifyDecisionMotivators(interactions) {
        const motivators = [];
        const elementTexts = interactions.map(i => i.elementText?.toLowerCase() || '').join(' ');
        // Common decision motivators
        if (elementTexts.includes('free shipping'))
            motivators.push('free_shipping');
        if (elementTexts.includes('discount') || elementTexts.includes('sale'))
            motivators.push('price_discount');
        if (elementTexts.includes('limited') || elementTexts.includes('exclusive'))
            motivators.push('scarcity');
        if (elementTexts.includes('review') || elementTexts.includes('rating'))
            motivators.push('social_proof');
        return motivators;
    }
    determineDominantPersonality(personalityTraits, decisionPatterns) {
        const sortedTraits = Object.entries(personalityTraits)
            .sort(([, a], [, b]) => b - a);
        return sortedTraits[0][0];
    }
    // Save psychology profile to database
    async savePsychologyProfile(profile) {
        try {
            await this.prisma.psychologyProfile.create({
                data: {
                    sessionId: profile.sessionId,
                    dominantPersonality: profile.dominantPersonality.toUpperCase(),
                    emotionalState: profile.emotionalState.toUpperCase(),
                    decisionMakingStyle: profile.decisionMakingStyle.toUpperCase(),
    ...
    // Update session with psychology insights
    async updateSessionWithPsychology(sessionId, profile) {
        try {
    ...
    async batchExtractPsychologyInsights(sessionIds) {
        ...
        for (const sessionId of sessionIds) {
            try {
                ...
                this.logger.error("Failed to extract psychology insights for session in batch", error, { sessionId });
            }
        }
    ...
    async getPsychologyInsights(sessionId) {
        try {
            ...
            return {
                sessionId: profile.sessionId,
                dominantPersonality: profile.dominantPersonality,
                ...
                pricesensitivity: profile.priceSensitivity,
                socialInfluence: profile.socialInfluence,
                insights: JSON.parse(profile.insights || '[]'),
                behaviorPredictions: JSON.parse(profile.behaviorPredictions || '[]'),
                recommendations: JSON.parse(profile.recommendations || '[]'),
                confidence: profile.confidence,
                processingTimestamp: profile.processingTimestamp
            };
        }
    ...
    async generatePsychologyTrainingData(sessionId) {
        try {
    ...
    // Generate training examples based on psychology insights
    generatePsychologyTrainingExamples(profile, session) {
        const examples = [];
    ...
    predictActionBasedOnPersonality(profile) {
    ...
    predictEmotionalResponse(profile, insights) {
    ...
    calculateEngagementLevel(profile) {
        const engagementScore = (profile.trustLevel +
            profile.urgencyLevel +
            (100 - profile.pricesensitivity) +
            profile.socialInfluence) / 4;
    ...
    identifyKeyDecisionFactors(profile) {
        const factors = [];
        if (profile.pricesensitivity > 70)
            factors.push('price');
        if (profile.trustLevel < 60)
            factors.push('trust_signals');
        if (profile.socialInfluence > 60)
            factors.push('social_proof');
        if (profile.urgencyLevel > 70)
    ...
    calculatePsychologyTrainingValue(profile) {
...
exports.PsychologyInsightsService = PsychologyInsightsService;
