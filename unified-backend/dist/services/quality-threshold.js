...
exports.QualityThresholdService = void 0;
...
class QualityThresholdService {
    ...
    constructor(prisma, validationService) {
        this.prisma = prisma;
        this.logger = new logger_1.Logger("QualityThreshold");
    ...
    async evaluateThresholds(validationResult) {
        const results = [];
    ...
    getRelevantScore(category, validationResult) {
        switch (category) {
            case 'overall':
    ...
    determineFinalAction(thresholdResults) {
    ...
    generateQualityRecommendations(validationResult, thresholdResults) {
    ...
    // Generate improvement suggestions
    generateImprovementSuggestions(validationResult, thresholdResults) {
        const suggestions = [];
        // Analyze patterns in failed thresholds
        const failedCategories = new Set(thresholdResults
            .filter(r => !r.passed)
    ...
    // Determine training eligibility
    determineTrainingEligibility(validationResult, thresholdResults) {
        ...
        const trainingThresholds = thresholdResults.filter(r => this.thresholds.get(r.thresholdId)?.category === 'training_readiness');
        return trainingThresholds.every(t => t.passed);
    }
    // Record quality trend data
    recordQualityTrend(assessment) {
        const trend = {
    ...
    // Update session status based on assessment
    async updateSessionStatus(sessionId, assessment) {
        try {
    ...
    generateThresholdMessage(threshold, actualScore, passed) {
        if (passed) {
    ...
    async getQualityMetrics(timeRange) {
        try {
            ...
            if (timeRange) {
                trends = trends.filter(t => t.timestamp >= timeRange.start && t.timestamp <= timeRange.end);
            }
    ...
    async assessMultipleSessions(sessionIds) {
        const assessments = [];
...
exports.QualityThresholdService = QualityThresholdService;
