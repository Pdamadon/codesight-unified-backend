...
class DataValidationService {
    ...
    constructor(prisma) {
        this.prisma = prisma;
        this.logger = new logger_1.Logger("DataValidation");
    ...
    async validateSession(sessionId) {
        ...
        try {
            ...
            // Calculate metrics
            const metrics = this.calculateValidationMetrics(validationResults, businessResults, Date.now() - startTime);
            const result = {
                sessionId,
        ...
        try {
            ...
            for (const rule of relevantRules) {
                ...
                try {
                    ...
                    this.logger.warn(`Validation rule ${rule.id} failed`, { error: (0, type_helpers_1.getErrorMessage)(error) });
                }
            }
    ...
    // Execute all validation rules
    async executeValidationRules(session) {
        const errors = [];
        const warnings = [];
        const scores = {};
        for (const [ruleId, rule] of this.validationRules) {
            ...
            try {
                ...
                this.logger.warn(`Validation rule ${ruleId} failed`, { error: (0, type_helpers_1.getErrorMessage)(error) });
                scores[ruleId] = 0;
            }
        }
    ...
    async executeBusinessRules(session) {
        const errors = [];
        const warnings = [];
        const scores = {};
        for (const [ruleId, rule] of this.businessRules) {
            ...
            try {
                ...
                this.logger.warn(`Business rule ${ruleId} failed`, { error: (0, type_helpers_1.getErrorMessage)(error) });
                scores[ruleId] = 0;
            }
        }
        return { errors, warnings, scores };
    ...
    calculateOverallScore(validationResults, businessResults) {
        const allScores = { ...validationResults.scores, ...businessResults.scores };
        const ruleWeights = new Map([...this.validationRules, ...this.businessRules]);
        let weightedSum = 0;
        let totalWeight = 0;
    ...
    calculateCategoryScores(validationResults, businessResults) {
        const allScores = { ...validationResults.scores, ...businessResults.scores };
        const ruleWeights = new Map([...this.validationRules, ...this.businessRules]);
    ...
    generateRecommendations(errors, warnings, session) {
    ...
    determineTrainingReadiness(overallScore, errors, session) {
    ...
    calculateValidationMetrics(validationResults, businessResults, duration) {
        const allErrors = [...validationResults.errors, ...businessResults.errors];
        const allWarnings = [...validationResults.warnings, ...businessResults.warnings];
        const allScores = { ...validationResults.scores, ...businessResults.scores };
    ...
    getRelevantRulesForDataType(dataType) {
        const relevantRules = [];
        switch (dataType) {
            case 'interaction':
                relevantRules.push(this.validationRules.get('interaction_structure'), this.validationRules.get('selector_quality'), this.businessRules.get('privacy_compliance'));
                break;
            case 'screenshot':
                relevantRules.push(this.validationRules.get('screenshot_structure'));
                break;
            case 'session_metadata':
                relevantRules.push(this.validationRules.get('session_basic_structure'), this.businessRules.get('privacy_compliance'));
                break;
        }
    ...
    // Batch validation
    async validateMultipleSessions(sessionIds) {
