...
exports.NavigationStrategyService = void 0;
...
class NavigationStrategyService {
    ...
    initializeAdditionalStrategies() {
    ...
    async identifyNavigationStrategy(sessionId) {
        try {
            ...
            const identifiedStrategy = await this.identifyPrimaryStrategy(navigationPattern, userPreferences, psychologyProfile, siteContext);
            // Find alternative strategies
            const alternativeStrategies = this.identifyAlternativeStrategies(identifiedStrategy, psychologyProfile, navigationPattern);
            // Generate site-specific insights
            const siteSpecificInsights = this.generateSiteSpecificInsights(siteContext, navigationPattern, identifiedStrategy);
            // Generate improvement suggestions
            const improvementSuggestions = this.generateImprovementSuggestions(identifiedStrategy, navigationPattern, siteContext);
            // Calculate confidence
            const confidence = this.calculateAnalysisConfidence(navigationPattern, psychologyProfile, session.interactions.length);
            const analysis = {
                sessionId,
    ...
    identifyUserPreferences(interactions, psychologyProfile) {
        ...
        let preferredStartingPoint = 'homepage';
        ...
        let informationGatheringStyle = 'moderate';
        ...
        let comparisonBehavior = 'limited_comparison';
        const productViews = interactions.filter(i => i.url?.includes('product') || i.url?.includes('item')).length;
        if (productViews <= 1) {
        ...
        let decisionMakingSpeed = 'deliberate';
        let trustBuildingNeeds = 'medium';
        let socialProofDependency = 'moderate';
    ...
    // Identify primary navigation strategy
    async identifyPrimaryStrategy(navigationPattern, userPreferences, psychologyProfile, siteContext) {
    ...
    customizeStrategy(baseStrategy, navigationPattern, userPreferences) {
    ...
    identifyAlternativeStrategies(primaryStrategy, psychologyProfile, navigationPattern) {
        const alternatives = [];
        // Get personality-aligned strategies
        if (psychologyProfile?.dominantPersonality) {
            const personalityStrategies = this.personalityNavigationMap.get(psychologyProfile.dominantPersonality.toLowerCase()) || [];
    ...
    // Generate site-specific insights
    generateSiteSpecificInsights(siteContext, navigationPattern, strategy) {
        const insights = [];
    ...
    // Generate improvement suggestions
    generateImprovementSuggestions(strategy, navigationPattern, siteContext) {
        const suggestions = [];
    ...
    calculateAnalysisConfidence(navigationPattern, psychologyProfile, interactionCount) {
        ...
        if (navigationPattern.uniquePagesVisited > 3)
            confidence += 10;
        if (navigationPattern.averageTimePerPage > 10000)
            confidence += 5;
        return Math.min(confidence, 100);
    }
    // Save navigation analysis to database
    async saveNavigationAnalysis(analysis) {
        try {
            ...
            if (existingInsights?.contextualInsights) {
                try {
                    insights = JSON.parse(existingInsights.contextualInsights);
                }
                catch (e) {
    ...
    async batchIdentifyNavigationStrategies(sessionIds) {
        ...
        for (const sessionId of sessionIds) {
            try {
                ...
                this.logger.error("Failed to identify navigation strategy for session in batch", error, { sessionId });
            }
        }
    ...
    async getNavigationStats() {
        try {
            const sessions = await this.prisma.unifiedSession.findMany({
                where: {
                    contextualInsights: { not: client_1.Prisma.AnyNull }
                },
                select: {
            ...
            const strategies = sessions
                .map(s => {
                try {
                    const insights = JSON.parse(s.contextualInsights);
    ...
    async generateNavigationTrainingData(sessionId) {
        try {
            const analysis = await this.identifyNavigationStrategy(sessionId);
            const trainingData = {
                sessionId,
    ...
    generateNavigationTrainingExamples(analysis) {
        const examples = [];
    ...
    calculateExpectedImpact(analysis) {
        if (analysis.navigationPattern.efficiency < 40)
            return 'high';
        if (analysis.navigationPattern.efficiency < 70)
            return 'medium';
        return 'low';
    }
    prioritizeRecommendations(recommendations) {
        // Simple prioritization based on keywords
        const highPriority = recommendations.filter(r => r.includes('search') || r.includes('navigation') || r.includes('trust'));
        const mediumPriority = recommendations.filter(r => r.includes('comparison') || r.includes('visual') || r.includes('mobile'));
        const lowPriority = recommendations.filter(r => !highPriority.includes(r) && !mediumPriority.includes(r));
        return [...highPriority, ...mediumPriority, ...lowPriority];
    }
    calculateNavigationTrainingValue(analysis) {
        let value = analysis.confidence * 0.4; // Base value from confidence
        // Pattern complexity adds value
        if (analysis.navigationPattern.complexity > 50)
            value += 15;
        if (analysis.navigationPattern.uniquePagesVisited > 5)
            value += 10;
...
exports.NavigationStrategyService = NavigationStrategyService;
