...
exports.UnifiedWebSocketServer = void 0;
...
class UnifiedWebSocketServer {
    ...
    constructor(server, dataProcessingPipeline) {
    ...
    async handleSessionStop(clientId, message) {
        const client = this.clients.get(clientId);
        if (!client || !client.authenticated) {
            this.sendError(clientId, 'Not authenticated');
            return;
    ...
    async handleInteractionEvent(clientId, message) {
        const client = this.clients.get(clientId);
    ...
    async handleScreenshotData(clientId, message) {
        const client = this.clients.get(clientId);
        if (!client || !client.authenticated || !client.sessionId) {
            this.sendError(clientId, 'No active session');
            return;
    ...
    async handleSessionComplete(clientId, message) {
        const client = this.clients.get(clientId);
        if (!client || !client.authenticated || !client.sessionId) {
            this.sendError(clientId, 'No active session');
            return;
        ...
        try {
            ...
            const result = await this.dataProcessingPipeline.completeSession(client.sessionId, message.data);
            this.sendToClient(clientId, {
    ...
    async handleSubscribeUpdates(clientId, message) {
        const client = this.clients.get(clientId);
        if (!client || !client.authenticated) {
            this.sendError(clientId, 'Not authenticated');
            return;
    ...
    subscribeToProcessingUpdates(clientId, sessionId, processingId) {
...
exports.UnifiedWebSocketServer = UnifiedWebSocketServer;
