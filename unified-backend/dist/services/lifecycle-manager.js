...
class LifecycleManager {
    ...
    storageManager;
    s3Storage;
    lifecyclePolicies = new Map();
    retentionPolicies = new Map();
    executionQueue = [];
    isRunning = false;
    constructor(prisma, storageManager) {
        this.prisma = prisma;
        this.logger = new logger_1.Logger("LifecycleManager");
    ...
    async executeLifecyclePolicies() {
        try {
            ...
            for (const policy of sortedPolicies) {
                try {
                    ...
                    results.errors.push(`Policy ${policy.name}: ${error.message}`);
                }
            }
    ...
    async getSessionsForLifecycleProcessing() {
    ...
    async executePolicyForSessions(policy, sessions) {
        const result = {
            rulesExecuted: 0,
            sessionsProcessed: 0,
            actionsPerformed: 0
        ...
        for (const rule of policy.rules.filter(r => r.enabled)) {
            const matchingSessions = sessions.filter(session => this.evaluateConditions(rule.conditions, session));
        ...
        return conditions.every(condition => {
            switch (condition.type) {
                case 'age':
                    ...
                    const targetAge = this.convertTodays(condition.value, condition.unit || 'days');
                    return this.compareValues(ageInDays, condition.operator, targetAge);
                case 'quality_score':
                    return this.compareValues(session.qualityScore || 0, condition.operator, condition.value);
                case 'training_value':
                    return this.compareValues(session.trainingValue || 0, condition.operator, condition.value);
                case 'file_size':
                    const archiveSize = session.archives?.[0]?.fileSize ? Number(session.archives[0].fileSize) : 0;
                    const targetSize = this.convertToBytes(condition.value, condition.unit || 'bytes');
    ...
    async executeActionsForSession(actions, session, policyName, ruleName) {
    ...
    async transitionStorage(session, targetStorageClass) {
    ...
    async notifyLifecycleEvent(session, action) {
        ...
        try {
            ...
            for (const [policyName, policy] of this.retentionPolicies) {
                ...
                try {
                    ...
                    result.errors.push(`Policy ${policyName}: ${error.message}`);
                }
            }
    ...
    async executeRetentionPolicy(policy) {
        ...
        const matchingSessions = sessions.filter(session => this.evaluateConditions(policy.conditions, session));
        ...
        try {
            ...
            for (const tempDir of tempDirs) {
                try {
                    ...
                    result.errors.push(`Directory ${tempDir}: ${error.message}`);
                }
            }
            result.duration = Date.now() - startTime;
    ...
    async cleanupDirectory(dirPath, maxAgeHours) {
        ...
        try {
            ...
            if (error.code !== 'ENOENT') { // Directory doesn't exist is OK
                throw error;
            }
        }
        return result;
    ...
    generateCostOptimizationRecommendations(costAnalysis, growthRate) {
    ...
    async executePolicy(policyName) {
