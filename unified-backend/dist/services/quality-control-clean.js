...
class QualityControlService {
    ...
    constructor() {
        ...
        this.logger = new logger_1.Logger("QualityControl");
    ...
    async validateSessionQuality(sessionId) {
        try {
    ...
    async calculateQualityMetrics(session) {
        ...
        const overallScore = Math.round((completeness * 0.3) +
            (reliability * 0.25) +
            (accuracy * 0.25) +
            (consistency * 0.2));
        return {
    ...
    calculateCompleteness(session) {
        ...
        if (session.screenshots && session.screenshots.length > 0) {
            ...
            if (session.screenshots.length >= 3)
                score += 10;
        }
        // Check for rich interaction data
        const richInteractions = session.interactions?.filter((i) => i.elementText && i.primarySelector && i.url).length || 0;
    ...
    calculateReliability(session) {
        ...
        session.interactions.forEach((interaction) => {
            if (interaction.primarySelector) {
                ...
                if (interaction.primarySelector.includes('[data-'))
                    selectorScore += 3;
                else if (interaction.primarySelector.includes('#'))
                    selectorScore += 2;
                else if (interaction.primarySelector.includes('.'))
                    selectorScore += 1;
    ...
    calculateAccuracy(session) {
        ...
        if (session.screenshots && session.screenshots.length > 0) {
            const qualityScreenshots = session.screenshots.filter((s) => s.quality > 70).length;
    ...
    calculateConsistency(session) {
        ...
        const timestamps = session.interactions.map((i) => Number(i.timestamp)).sort();
        ...
        const domains = new Set(session.interactions
            .map((i) => {
            try {
            ...
            .filter(Boolean));
        if (domains.size === 1)
            score += 10; // Single domain
        else if (domains.size <= 2)
            score += 5; // Max 2 domains
        ...
        if (interactionTypes.size >= 2)
            score += 5; // Multiple interaction types is good
        return Math.min(score, 100);
    }
    checkUrlConsistency(interactions) {
    ...
    identifyQualityIssues(session, metrics) {
        const issues = [];
    ...
    async saveQualityReport(sessionId, metrics, issues, recommendations) {
        try {
    ...
    async getQualityStats() {
        const [totalReports, avgScore, scoreDistribution, recentReports] = await Promise.all([
        ...
        scoreDistribution.forEach(item => {
            ...
            if (score >= this.thresholds.excellent)
                gradeDistribution.excellent += item._count;
            else if (score >= this.thresholds.good)
                gradeDistribution.good += item._count;
            else if (score >= this.thresholds.minimum)
                gradeDistribution.acceptable += item._count;
            else
                gradeDistribution.poor += item._count;
        });
        return {
            totalReports,
    ...
    async scoreInteraction(interaction) {
        try {
            let score = 0;
            ...
            if (interaction.primarySelector && interaction.primarySelector.length > 0) {
                ...
                if (interaction.primarySelector.startsWith('#'))
                    score += 10;
                else if (interaction.primarySelector.includes('.'))
                    score += 5;
            }
            // Score based on confidence
            if (interaction.confidence && interaction.confidence > 0.8)
                score += 15;
            else if (interaction.confidence && interaction.confidence > 0.6)
                score += 10;
            else if (interaction.confidence && interaction.confidence > 0.4)
                score += 5;
            return Math.min(score, 100);
        }
        catch (error) {
