...
const data_validation_1 = require("./data-validation");
const context_enhancement_1 = require("./context-enhancement");
const psychology_insights_1 = require("./psychology-insights");
const navigation_strategy_1 = require("./navigation-strategy");
const parallel_processing_manager_1 = require("./parallel-processing-manager");
class DataProcessingPipeline extends events_1.EventEmitter {
    prisma;
    storageManager;
    openaiService;
    qualityControl;
    ...
    jobQueue = [];
    activeJobs = new Map();
    processingUpdateCallbacks = new Map();
    isProcessing = false;
    maxConcurrentJobs = 5;
    processingInterval = null;
    constructor(prisma, storageManager, openaiService, qualityControl) {
        super();
        this.prisma = prisma;
        this.storageManager = storageManager;
        this.openaiService = openaiService;
        this.qualityControl = qualityControl;
        this.dataValidation = new data_validation_1.DataValidationService(prisma);
        this.contextEnhancement = new context_enhancement_1.ContextEnhancementService(prisma, openaiService);
        this.psychologyInsights = new psychology_insights_1.PsychologyInsightsService(prisma, openaiService);
        this.navigationStrategy = new navigation_strategy_1.NavigationStrategyService(prisma, openaiService);
        this.parallelProcessing = new parallel_processing_manager_1.ParallelProcessingManager(prisma);
        this.logger = new logger_1.Logger('DataProcessingPipeline');
    ...
    async validateStreamData(data, dataType) {
        try {
    ...
    async processInteraction(interactionData) {
        ...
        try {
            ...
            return {
                id: jobId,
                status: 'error',
                error: (0, type_helpers_1.getErrorMessage)(error)
            };
        }
    }
    ...
    async processScreenshot(screenshotData) {
        ...
        try {
            ...
            const compressionResult = await this.storageManager.compressAndUploadScreenshot(screenshotData.dataUrl, screenshotData.sessionId, screenshotData.eventType);
    ...
    async processJob(job) {
        ...
        try {
            ...
            job.status = 'failed';
            job.error = (0, type_helpers_1.getErrorMessage)(error);
            job.retryCount++;
    ...
    // Processing Methods
    async enhanceInteraction(interactionId) {
        ...
        if (interaction.relatedScreenshots.length > 0) {
            const visionAnalysis = await this.openaiService.analyzeScreenshots(interaction.relatedScreenshots);
    ...
    async processCompleteSession(sessionId, data) {
    ...
    async performQualityCheck(sessionId) {
        return await this.qualityControl.assessSession(sessionId);
    }
    async generateTrainingData(sessionId) {
        ...
        // Save training data
        const jsonlContent = trainingData.messages.map((msg) => JSON.stringify(msg)).join('\n');
    ...
    async extractPsychologyInsights(sessionId) {
        try {
    ...
    // Event Handling
    onProcessingUpdate(processingId, callback) {
        this.processingUpdateCallbacks.set(processingId, callback);
    }
    notifyProcessingUpdate(processingId, update) {
    ...
    async processSessionsInParallel(sessionIds, priority = 3) {
    ...
    async waitForParallelJobs(jobIds, timeout = 600000) {
    ...
    async processSessionHighPriority(sessionId) {
    ...
    async batchProcessOperations(operations) {
        const jobIds = [];
        for (const operation of operations) {
            ...
            switch (operation.type) {
                case 'validation':
                    ...
                    const validationJobs = await this.parallelProcessing.processSessions(operation.sessionIds, operation.priority || 4);
                    jobIds.push(...validationJobs);
                    break;
                case 'quality_scoring':
                    jobId = await this.parallelProcessing.batchQualityScoring(operation.sessionIds, operation.priority || 4);
                    jobIds.push(jobId);
                    break;
                case 'context_enhancement':
                    jobId = await this.parallelProcessing.batchContextEnhancement(operation.sessionIds, operation.priority || 4);
                    jobIds.push(jobId);
                    break;
            }
        }
    ...
    getEnhancedStatus() {
        ...
        return {
            ...
            combined: {
                ...
                utilizationRate: Math.round(((traditionalStatus.activeJobs + parallelStats.busyWorkers) /
