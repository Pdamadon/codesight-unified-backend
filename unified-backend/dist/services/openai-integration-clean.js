...
class OpenAIIntegrationService {
    ...
    async analyzeScreenshotWithVision(screenshot) {
...
Focus on psychological insights that would help understand the user's shopping mindset.`;
    ...
    extractTrustLevel(analysis) {
        let score = 50; // baseline
        if (analysis.includes('trust') || analysis.includes('secure') || analysis.includes('verified'))
            score += 20;
        if (analysis.includes('suspicious') || analysis.includes('doubt') || analysis.includes('uncertain'))
            score -= 20;
        if (analysis.includes('review') || analysis.includes('rating') || analysis.includes('testimonial'))
            score += 15;
        return Math.max(0, Math.min(100, score));
    }
    extractUrgencyLevel(analysis) {
        let score = 30; // baseline
        if (analysis.includes('urgent') || analysis.includes('limited') || analysis.includes('hurry'))
            score += 30;
        if (analysis.includes('sale') || analysis.includes('discount') || analysis.includes('offer'))
            score += 20;
        if (analysis.includes('relaxed') || analysis.includes('browsing') || analysis.includes('casual'))
            score -= 15;
        return Math.max(0, Math.min(100, score));
    }
    extractPriceSensitivity(analysis) {
        let score = 40; // baseline
        if (analysis.includes('price') || analysis.includes('cost') || analysis.includes('cheap'))
            score += 25;
        if (analysis.includes('expensive') || analysis.includes('budget') || analysis.includes('afford'))
            score += 20;
        if (analysis.includes('premium') || analysis.includes('luxury') || analysis.includes('quality'))
            score -= 15;
        return Math.max(0, Math.min(100, score));
    }
    extractSocialInfluence(analysis) {
        let score = 35; // baseline
        if (analysis.includes('review') || analysis.includes('rating') || analysis.includes('social'))
            score += 25;
        if (analysis.includes('popular') || analysis.includes('trending') || analysis.includes('recommend'))
            score += 20;
        if (analysis.includes('independent') || analysis.includes('personal') || analysis.includes('own'))
            score -= 15;
        return Math.max(0, Math.min(100, score));
    }
    ...
    generateBehaviorPredictions(analysis) {
        const predictions = [];
    ...
    calculateVisionQualityScore(analysis) {
        ...
        if (analysis.length > 200)
            score += 15;
        if (analysis.includes('user') || analysis.includes('customer'))
            score += 10;
        if (analysis.includes('psychology') || analysis.includes('behavior'))
            score += 15;
        if (analysis.includes('trust') || analysis.includes('emotion'))
            score += 10;
        return Math.max(0, Math.min(100, score));
    }
    calculateConfidence(analysis) {
        ...
        if (analysis.includes('clearly') || analysis.includes('obviously'))
            confidence += 15;
        if (analysis.includes('appears') || analysis.includes('seems'))
            confidence -= 10;
        if (analysis.includes('uncertain') || analysis.includes('unclear'))
            confidence -= 20;
    ...
    async generateTrainingData(sessionData) {
        try {
    ...
    async formatForOpenAI(sessionData) {
        const messages = [];
    ...
    generateContextualResponse(interaction, psychology) {
        const responses = [
    ...
    calculateTrainingValue(sessionData) {
        let value = 50; // baseline
        if (sessionData.interactions?.length > 5)
            value += 20;
        if (sessionData.screenshots?.length > 3)
            value += 15;
        if (sessionData.userPsychology?.confidence > 70)
            value += 15;
        if (sessionData.qualityScore > 80)
            value += 20;
        return Math.max(0, Math.min(100, value));
    }
    calculateComplexity(sessionData) {
        let complexity = 30; // baseline
        if (sessionData.interactions?.length > 10)
            complexity += 25;
        if (sessionData.pageStructure?.complexity > 0.7)
            complexity += 20;
        if (sessionData.navigationStrategy?.efficiency < 0.5)
            complexity += 15;
        return Math.max(0, Math.min(100, complexity));
    }
    // File Management
    async uploadTrainingFile(data, metadata) {
        try {
            const jsonlContent = data.messages.map((msg) => JSON.stringify(msg)).join('\n');
