...
class QualityControlService {
    ...
    async scoreInteraction(interaction) {
        ...
        // Visual information
        if (interaction.boundingBox && interaction.boundingBox !== '{}')
            score += 10;
        if (interaction.viewport && interaction.viewport !== '{}')
            score += 5;
        // State tracking
        if (interaction.stateBefore && interaction.stateBefore !== '{}')
            score += 5;
        if (interaction.stateChanges && interaction.stateChanges !== '{}')
            score += 10;
        // User intent (from audio/reasoning)
        if (interaction.userIntent)
            score += 10;
        if (interaction.userReasoning)
            score += 5;
        return Math.min(score, maxScore);
    }
    // Evaluate selector quality and reliability
    evaluateSelectorQuality(interaction) {
        let score = 0;
        ...
        if (interaction.cssPath)
            score += 10;
        return Math.min(score, 100);
    }
    // Evaluate context richness
    evaluateContextRichness(interaction) {
        let score = 0;
        // DOM context
        try {
    ...
    assessCompleteness(session, issues) {
        let score = 100;
    ...
    assessReliability(session, issues) {
        let score = 100;
        const interactions = session.interactions || [];
    ...
    generateRecommendations(session, issues, recommendations) {
    ...
    async batchAssessQuality(sessionIds) {
        const reports = [];
    ...
    async assessScreenshotQuality(screenshot) {
        let score = 0;
        const maxScore = 100;
        try {
    ...
    async testSelectorReliability(selector, sessionId) {
        try {
    ...
    // Check for dynamic content indicators
    hasDynamicContentIndicators(selector) {
    ...
    async getRecentScreenshots(sessionId, timeWindow) {
    ...
    async calculateOverallSessionQuality(sessionId) {
        try {
            const session = await this.prisma.unifiedSession.findUnique({
                where: { id: sessionId },
                include: {
            ...
            if (session.screenshots.length > 0) {
                const screenshotScores = await Promise.all(session.screenshots.map(screenshot => this.assessScreenshotQuality(screenshot)));
            ...
            if (clickInteractions.length > 0) {
                const selectorScores = await Promise.all(clickInteractions.map(interaction => this.testSelectorReliability(interaction.primarySelector, sessionId)));
    ...
    // Calculate training value based on shopping behavior patterns
    calculateTrainingValue(session) {
        ...
        const hasCartActions = interactions.some(i => i.elementText?.toLowerCase().includes('cart') ||
            i.elementText?.toLowerCase().includes('add to cart'));
        const hasSearchBehavior = interactions.some(i => i.type === 'INPUT' ||
    ...
    async batchCalculateQuality(sessionIds) {
